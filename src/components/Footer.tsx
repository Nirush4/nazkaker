...
import { IconBrandInstagram, IconBrandFacebook, IconBrandPinterest, IconMapPin } from '@tabler/icons-react'
...
export default function Footer() {
  return (
    <footer className="border-t border-beige bg-beige/20 mt-auto">
      <div className="max-w-7xl mx-auto px-6 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 items-start text-center md:text-left">
          <div className="flex flex-col items-center md:items-start gap-4">
            <Link to="/">
              <img src={logo} alt="Naz Kaker" className="h-9" />
            </Link>
            <p className="text-warm-brown text-sm leading-relaxed max-w-xs">
              Håndlagde kaker og cookies på bestilling – bakt med kjærlighet til hver anledning.
            </p>
          ...
          <div>
            <h3 className="font-serif text-lg text-chocolate mb-4">Hurtiglenker</h3>
            <ul className="space-y-2">
              {quickLinks.map(({ to, label }) => (
                <li key={to}>
                  <Link
                    to={to}
                    className="text-warm-brown text-sm hover:text-chocolate transition-colors duration-200"
                  >
                    {label}
                  </Link>
                </li>
          ...
          <div className="flex flex-col items-center md:items-start">
            <h3 className="font-serif text-lg text-chocolate mb-4">Følg oss</h3>
            <div className="flex gap-4">
              {socialLinks.map(({ href, label, icon: Icon }) => (
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="text-warm-brown hover:text-chocolate transition-colors duration-200"
                >
